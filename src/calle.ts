...
export async function executeWithCalle(
  ...
  const task = isAppointmentCall
    ? [
        ...
        "If the patient says they cannot attend the current appointment, do NOT ask the unnatural question 'Czy chciałby pan przełożyć wizytę na inny termin?'. Instead respond like a receptionist: 'Rozumiem. W takim razie mogę sprawdzić najbliższy wolny termin.' Then offer the earliest prepared replacement slot with its date and time.",
        ...
        "If the patient rejects the offered time, do not immediately ask a formal multi-part question. Say naturally: 'Rozumiem. A sam dzień panu odpowiada, tylko godzina nie?' If the day works, ask: 'Która godzina byłaby dla pana dogodna?' Accept the requested time only if that exact date/time exists in the prepared availability. If it does not exist, say that this hour is unfortunately unavailable and offer the available times for that same day.",
