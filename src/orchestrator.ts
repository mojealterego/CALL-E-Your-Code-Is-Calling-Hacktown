...
export async function runIncident(incident: Incident, options: { live: boolean; ledger?: AuditLedger } = { live: false }) {
  ...
  if (!policy.allowed) { const record = ledger.transition(operationKey, "escalated", { outcome: failureOutcome(policy.reasons.join("; ")), transactionDecision: "abort", transactionReasons: policy.reasons }); return { record, reused: false, outcome: record.outcome }; }
  ledger.transition(operationKey, "validated");
  const appointment = incident.appointment;
  const isAppointment = appointment !== undefined;
  let transaction: PreparedAppointmentTransaction | PreparedTransaction;
  try {
    transaction = isAppointment ? prepareAppointmentTransaction({ transactionId: `TX-${incident.id}-${incident.vehicleId}`, incidentId: incident.id, participantId: incident.vehicleId, constraints: appointment }) : prepareTransaction({ transactionId: `TX-${incident.id}-${incident.vehicleId}`, incidentId: incident.id, participantId: incident.vehicleId, route: incident.proposedRoute, maxEta: incident.maxEta });
  } catch (error) {
    const message = error instanceof Error ? error.message : "Invalid prepared transaction";
    const record = ledger.transition(operationKey, "escalated", { outcome: failureOutcome(message), transactionDecision: "abort", transactionReasons: [message] });
  ...
  if (!handshake.accepted) { const reasons = [`provider handshake rejected: ${handshake.reasons.join("; ")}`]; const record = ledger.transition(operationKey, "recovering", { outcome: failureOutcome(reasons[0] ?? "Provider handshake rejected"), transactionDecision: "recover", transactionReasons: reasons }); return { record, reused: false, outcome: record.outcome }; }
