...
export async function executeWithCalle(
  ...
  const task = isAppointmentCall
    ? [
        ...
        "Opening control: the opening is exactly ONE spoken turn. Say the greeting/introduction once, then stop and wait for the patient's response. Never repeat, restart, paraphrase, or prepend another greeting. If there is silence, interruption, barge-in, latency, or turn-taking uncertainty, do not restart the greeting; continue from the current conversation state and ask only the next appropriate short question.",
        "Do not generate any separate generic greeting before or after the opening defined above.",
