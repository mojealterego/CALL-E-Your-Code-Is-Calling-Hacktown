...
export async function executeWithCalle(
  ...
  const task = isAppointmentCall
    ? [
        ...
        "If the patient rejects the offered time, say exactly: 'Rozumiem. A sam dzień panu odpowiada, tylko godzina nie?' If the day works, ask exactly: 'Która godzina byłaby dla pana dogodna?' Accept the requested time only if that exact date/time exists in the prepared availability. If it does not exist, say that this hour is unfortunately unavailable and offer the available times for that same day.",
