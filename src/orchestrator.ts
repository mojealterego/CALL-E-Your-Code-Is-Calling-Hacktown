...
export async function runIncident(incident: Incident, options: { live: boolean; ledger?: AuditLedger } = { live: false }) {
  ...
  try {
    ...
    const observedEvidence = {
      route: outcome.route, eta: outcome.eta_update_time, acceptance: outcome.route_acceptance, confidence: outcome.confidence, evidenceSummary: outcome.evidence_summary, providerStatus: raw.status,
      ...(outcome.evidence !== undefined ? { evidenceItems: outcome.evidence } : {}), ...(outcome.task_completed !== undefined ? { taskCompleted: outcome.task_completed } : {}), ...(outcome.completion_confidence !== undefined ? { completionConfidence: outcome.completion_confidence } : {}),
      ...(outcome.patient_confirmed !== undefined ? { patientConfirmed: outcome.patient_confirmed } : {}), ...(outcome.appointment_confirmed !== undefined ? { appointmentConfirmed: outcome.appointment_confirmed } : {}), ...(outcome.doctor_confirmed !== undefined ? { doctorConfirmed: outcome.doctor_confirmed } : {}), ...(outcome.first_visit !== undefined ? { firstVisit: outcome.first_visit } : {}),
      ...(outcome.identity_document_reminder_given !== undefined ? { identityDocumentReminderGiven: outcome.identity_document_reminder_given } : {}), ...(outcome.arrive_30_minutes_early !== undefined ? { arrive30MinutesEarly: outcome.arrive_30_minutes_early } : {}), ...(outcome.registration_reminder_given !== undefined ? { registrationReminderGiven: outcome.registration_reminder_given } : {}), ...(outcome.information_form_reminder_given !== undefined ? { informationFormReminderGiven: outcome.information_form_reminder_given } : {}),
      ...(outcome.conversation_completed !== undefined ? { conversationCompleted: outcome.conversation_completed } : {}), ...(outcome.appointment_decision !== undefined ? { appointmentDecision: outcome.appointment_decision } : {}), ...(outcome.reschedule_requested !== undefined ? { rescheduleRequested: outcome.reschedule_requested } : {}), ...(outcome.reschedule_completed !== undefined ? { rescheduleCompleted: outcome.reschedule_completed } : {}), ...(outcome.new_appointment_date !== undefined ? { newAppointmentDate: outcome.new_appointment_date } : {}), ...(outcome.new_appointment_time !== undefined ? { newAppointmentTime: outcome.new_appointment_time } : {}),
    };
