...
import {
  adversarialDecisionChecks,
  buildCapabilityAdapter,
  buildDecisionReceipt,
  buildFailureMemory,
  clarifyAppointmentResponse,
  classifyAutonomy,
  decisionMatrix,
  negotiatePreparedOptions,
  runMetaArchitectDecisionLoop,
  scoreRiskConfidence,
  trajectoryMetrics,
} from "../src/meta-architect-decision-loop.js";

const now = "2026-09-14T06:00:00.000Z";
function claim(predicate: string, value: string, id: string): Claim {
  return createClaim({
...
describe("Meta-Architect Decision Loop", () => {
  it("negotiates only prepared options and never invents a slot", () => {
    const options = [
      { id: "s1", date: "2026-09-15", time: "09:00", prepared: true, preferenceScore: 0.9, riskScore: 0.1 },
      { id: "s2", date: "2026-09-15", time: "11:30", prepared: true, preferenceScore: 0.8, riskScore: 0.1 },
    ];
  ...
  it("fails closed when a formal prerequisite is missing", () => {
    expect(decisionMatrix({
      identityVerified: true, appointmentVerified: true, providerTerminal: true,
      taskCompleted: true, conversationCompleted: true, evidencePresent: true,
      noContradiction: false, slotPrepared: true, trajectorySafe: true,
    }).action).toBe("recover");
    expect(adversarialDecisionChecks({
      identityVerified: false, appointmentVerified: false, slotPrepared: false,
      stateFresh: false, callKnown: false, evidencePresent: false,
      contradiction: true, authoritativeReadback: false,
    })).toHaveLength(8);
  });
  ...
  it("keeps trajectory safety and failure memory explicit", () => {
    const events: TrajectoryEvent[] = [
      ...safeTrajectory,
      { type: "retry", detail: "retry after commit", safe: false },
    ];
  ...
  it("scores evidence and creates tamper-evident decision receipt", () => {
    ...
    expect(score.risk).toBeLessThan(0.3);
  ...
  it("runs the complete commit path only with verified claims and a known capability", () => {
    ...
    const result = runMetaArchitectDecisionLoop({
      intent: "confirm appointment",
      patientName: "Adam Miauczyński",
      doctorName: "dr Pawlak",
      ...
      evidenceItems: ["patient identity", "appointment decision", "doctor", "conversation completed"],
      selectedSlotPrepared: true,
      contradiction: false,
      capability,
    });
    expect(result.action).toBe("commit");
    expect(result.allowed).toBe(true);
    expect(result.receipt?.action).toBe("commit");
  });
  ...
  it("recovers on contradiction instead of committing", () => {
    const result = runMetaArchitectDecisionLoop({
      ...
      selectedSlotPrepared: true,
      contradiction: true,
      capability: buildCapabilityAdapter({ provider: "CALL-E", operation: "appointment-confirmation", participant: "Adam Miauczyński", endpoint: "+48518873940", constraints: ["confirm appointment"] }),
    });
    expect(result.allowed).toBe(false);
    expect(result.action).toBe("recover");
    expect(result.adversarialChecks.some((x) => x.includes("CONTRADICTION"))).toBe(true);
  });
});
