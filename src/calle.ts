...
export async function executeWithCalle(
  ...
  const task = isAppointmentCall
    ? [
        ...
        "After the patient is confirmed, say exactly: 'Dzwonię w sprawie potwierdzenia pana jutrzejszej wizyty u doktora Pawlaka. Czy potwierdza pan?'",
        "If the patient confirms the current appointment, ask exactly: 'Dobrze. Czy będzie to pana pierwsza wizyta w naszej przychodni czy kolejna?'",
        "If it IS the first visit, say exactly: 'W takim razie przypominam, żeby zabrał pan ze sobą dowód osobisty lub inny dokument potwierdzający tożsamość i przyjechał około 30 minut wcześniej. Przed pierwszą wizytą trzeba zgłosić się do rejestracji i wypełnić formularz informacyjny.'",
        ...
        "After either attendance branch, ask exactly: 'Czy jest coś, w czym jeszcze mogę pomóc?' If the patient has no questions, close exactly: 'Dziękuję za rozmowę i życzę miłego dnia.'",
