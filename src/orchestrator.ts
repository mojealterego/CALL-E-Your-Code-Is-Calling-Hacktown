...
export async function runIncident(
  ...
  if (reserved.state !== "detected") {
    return { record: reserved, reused: true, outcome: reserved.outcome, reflexion: undefined, assurance: undefined };
  }
  ...
  try {
    ...
    const state = canResolve(outcome) && claims.commitAllowed && monitorability.overall >= 0.9 ? "resolved" : "escalated";
    if (state === "escalated") trajectory.push({ type: "escalation", detail: "assurance gate did not permit commit", safe: true });
